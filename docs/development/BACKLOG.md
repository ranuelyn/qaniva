...
| QAN-008 | iOS **device** run + lifecycle hardening | MOB+UNI | **BLOCKED** (2026-08-31) | The paired iPhone 14 Pro Max ("Yusuf Asım iPhone'u", iOS 26.3.1) stayed `unavailable` in devicectl for the whole sprint — never connected, so no device build was attempted (a valid "Apple Development: yusufasimarslan@gmail.com" signing identity exists, so signing is ready). **To unblock:** connect the iPhone via cable (or enable Wi-Fi debugging), unlock it, tap "Trust This Computer", then re-run the sprint's device steps: `scripts/export-unity-ios.sh` (device SDK), `xcodebuild ... -destination 'platform=iOS' -allowProvisioningUpdates`, install + run, verify the interactive 3D slice + second launch. | QAN-004 |
